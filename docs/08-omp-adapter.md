...
첨부는 OMP의 임의 `attachments` 필드로 보내지 않는다. 이미지 첨부는 `ImageContent { type:"image", data:<base64>, mimeType }`로 변환해 `prompt.images` 또는 `steer.images`에 전달한다. 파일·폴더·코드 선택은 작업 root 경계를 다시 검증한 뒤 경로, 코드 범위, 실제 텍스트 내용을 `message`의 `<awi_context>` 블록으로 직렬화한다. 다른 작업의 attachment ID, 손상된 코드 선택 메타데이터, 지원하지 않는 이미지/비 UTF-8 텍스트는 명시적으로 실패시킨다.
