...
2026-09-23. 각 작업은 구현 에이전트가 수행한다. IMP-01 기초 환경을 구성했으며 IMP-02 이후의 앱·연동 코드는 아직 없다. 실행 범위는 [도구 버전·기초 검증](evidence/toolchain.md)을 따른다.
...
첫 묶음은 IMP-01, IMP-02, IMP-03이다. 결과는 선택 버전·검증 로그·필수 기능 대응표이며 완제품이 아니다. 문제가 없으면 IMP-04~07로 진행하고 나머지는 의존표에 따른다. 런타임·확장 정확한 버전은 이 묶음의 실제 설치 결과로 잠근다.
