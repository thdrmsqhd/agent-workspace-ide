...
2026-09-23. 각 작업은 구현 에이전트가 수행한다. IMP-01 기초 환경을 구성했고 IMP-05의 명령 봉투·CMD-06~10 입력 검사와 IMP-07의 영속성 없는 큐·슬롯 규칙을 일부 구현했다. IMP-02/03 연동 검증은 사용자 지시에 따라 후속으로 미룬다. 실제 앱·DB·연동 코드는 아직 없다. 실행 범위는 [도구 버전·기초 검증](evidence/toolchain.md)을 따른다.
...
현재 진행 순서는 IMP-01 뒤 IMP-05의 독립 계약과 IMP-07의 순수 규칙을 먼저 구현하는 것이다. IMP-07 전체 완료에는 IMP-06 트랜잭션·영속성이 필요하다. IMP-02/03의 IDE·OMP 실제 연동 검증과 해당 버전 고정은 후속으로 보류한다. 검증하지 않은 런타임·확장 버전을 임의로 잠그지 않는다.
