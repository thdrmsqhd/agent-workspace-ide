...
| 004,031,032 | `packages/core`의 논의 도구 제한·수동/자동 승인·스냅샷/HEAD/정책 해시 판정 | OMP 도구 경계 실제 강제, 지침 해석·정책 스냅샷 생성, 승인·반영 서비스 |
...
| 007,012,013,016,017 | `packages/core`의 12슬롯·대기열·Esc 규칙, SQLite의 수정·선점 원자성 | 12개 실제 동시 실행, 엔진 전달·자손 중단·재개, UI 즉시/대기 입력 수용 |
...
마지막 행의 REQ-048은 후보 평가 문서가 있어도 실제 IDE 기반의 80% 유사도 판정이 확정된 것은 아니다. REQ-001/018/022/023의 VS Code 확장·IDE 유지와 REQ-007/016의 실제 다중 에이전트·중단은 선택 IDE/OMP 버전을 정하고 연동 검증해야만 완료 가능하다. 사용자의 연동 보류 지시에 따라 해당 검증을 수행하지 않았다.
