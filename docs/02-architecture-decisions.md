...
| ADR-003 | OMP 별도 프로세스 RPC 어댑터, 다른 엔진과 계약 분리 | 조건부, 18.2.5 RPC 검증 완료·자손 중단 미해결 | TV-003/004/007 |
| ADR-004 | 화면 선택과 작업 실행·IDE 서비스 수명을 분리 | 채택, 호스트 구현은 조건부 | TV-001/002 |
| ADR-005 | 수정 가능한 후속 지시 대기열은 앱이 소유 | 채택 | TV-005 |
| ADR-006 | 논의 단계 도구 제한과 외부 변경 범위를 실행 경계에서 적용 | 채택, 18.2.5 논의 구성 검증 완료 | TV-006/014 |
...
이 중 contracts/core/persistence/worktrees/ui와 추가된 diff/files 일부를 구현했다. desktop/engine-omp/processes/ide-integration은 아직 구현하지 않았다. 구체적인 API 타입·오류 코드는 07을 단일 기준으로 관리한다.
...
공개 RPC 문서에서 확인한 연결 지점은 prompt, steer, abort, get_state, set_subagent_subscription, get_subagents 등이다. 실제 18.2.5에서 TV-003으로 연결·프레임·요청 ID를 통과했고([결과](../docs/evidence/TV-003/README.md)), TV-004에서 트리·구독은 동작하나 `abort`가 하위 에이전트 셸을 멈추지 않음을 확인했다. 따라서 어댑터 능력은 `abortTreeVerified=false`로 기록하고, 자손 중단 경로를 확보하기 전에는 중단을 완료로 보고하지 않는다.

앱 대기열은 ADR-005를 사용한다. 큰 출력은 점진적으로 처리하며 stderr와 stdout 프로토콜을 혼합하지 않는다. 요청 ID는 상관관계용이며 엔진이 중복 실행 방지를 보장한다는 뜻이 아니다. 실측상 실패 응답에는 `id`가 없을 수 있으므로 앱은 id 결합만으로 오류를 처리하지 않고 자체 타임아웃과 미귀속 오류 격리를 둔다.
...
18.2.5 실측(TV-006)에서 확정한 논의 모드 구성은 도구 목록 `read`·`glob`·`grep`·`todo` + 승인 정책 `write`·`edit`·`bash`·`eval`·`task`·`hub`·`web_search` 거부 + 프로젝트 MCP 설정 차단(`mcp.enableProjectConfig=false`)이다. 도구 목록만 제한하면 MCP 쓰기 장치가 계속 실행되므로 세 가지를 함께 적용한다. 근거는 [TV-006 결과](../docs/evidence/TV-006/README.md)에 있다.
...
상세 계약은 [문서 색인](00-document-index.md)의 04~19와 04a에 작성되어 있다. 화면은 04/04a, 저장은 06, API는 07, 정책은 13, 빌드 계약은 18이 권위자다. 실제 구현 범위는 19에 별도로 기록한다.
