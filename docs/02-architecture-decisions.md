...
중단은 메인·자손 에이전트 및 그 순간 실행 중인 단기 명령에 전달한다. 별도로 등록된 개발 서버는 유지한다. 작업 전환은 어떤 실행 그룹도 종료하지 않는다. 앱 종료 시 소유 프로세스 처리 정책은 OPEN-006을 따른다.
...
아직 이 문서가 대체하지 않는 후속 문서: API의 필드·오류 코드, 실제 DB 스키마, 화면별 상호작용, 워크플로우 지침 우선순위, 빌드 명령. 구현자는 해당 상세 계약이 필요한 경계에서 임의의 별도 계약을 확산시키지 말고 이 ADR의 방향으로 한 곳에 정의해야 한다.
